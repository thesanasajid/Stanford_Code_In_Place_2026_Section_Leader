# Stanford Code in Place 2026, Section Leader 🌲

![Stanford](https://img.shields.io/badge/Stanford-Code%20in%20Place-red)
![Python](https://img.shields.io/badge/Python-Programming-blue)

## About Me

Hi! I'm **Sana Sajid**, a **Section Leader** for **Stanford University's Code in Place 2026** program.

I am a **Nutritionist** with a strong interest in **Artificial Intelligence (AI), Machine Learning (ML), Big Data, and Public Health Research**.

### Interests

- Applying AI/ML in nutrition and healthcare
- Analyzing dietary patterns and health outcomes
- Building predictive and risk assessment models
- Developing personalized nutrition solutions
- Using big data for public health insights
- Promoting evidence-based nutrition and preventive care

---

## Repository Contents

- Weekly section materials  
- Stanford Karel solutions  
- Python assignments  
- Graphics programs  
- Lists & Dictionaries projects  
- Teaching resources from Code in Place 2026  

---

## About Stanford Code in Place

**Code in Place** is a global programming initiative inspired by Stanford’s CS106A course.

It teaches Python through:

- Live interactive sections  
- Problem-solving exercises  
- Community mentorship  
- Beginner-friendly learning  

Official Website: https://codeinplace.stanford.edu

---

## My Role

As a Section Leader, I:

- Conducted weekly live coding sessions  
- Mentored international students  
- Helped with debugging and problem solving  
- Led breakout room discussions  
- Supported beginner programmers  
- Promoted collaborative learning  

---

## Weekly Structure

| Week | Topic |
|------|------|
| Week 1 | Hospital Karel |
| Week 2 | Art of Karel |
| Week 3 | Intro to Python |
| Week 4 | Control Flow |
| Week 5 | Graphics |
| Week 6 | Lists & Dictionaries |

---

## Global Teaching Experience

Students from:

- USA
- Peru
- Argentina
- Nigeria
- Germany
- Mexico
- India

---

## TeachNow Sessions

Conducted **7 TeachNow sessions** on:

- Debugging  
- Problem solving  
- Logic building  
- Assignment support  
- Beginner mentorship  

---

## Skills

- Python Programming  
- Teaching & Mentorship  
- Debugging  
- Problem Solving  
- GitHub Management  
- Public Speaking  
- Collaborative Learning  

---

## Technologies

- Python  
- GitHub  
- Stanford Karel  
- CS106A  

---

## Certification

**Role:** Section Leader  
**Program:** Stanford Code in Place 2026  

---

## Connect

LinkedIn: linkedin.com/in/the-sana-sajid 
Gmail:: sana.sajid547@gmail.com

---

## Acknowledgment

- Stanford University  
- Code in Place Team  
- Head TAs & Fellow Section Leaders

---

## Happy Coding 🚀

Programming is a journey of curiosity, consistency, and creativity.
```
